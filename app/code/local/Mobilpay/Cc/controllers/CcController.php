...
class Mobilpay_Cc_CcController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        /** @var Mobilpay_Cc_Model_Recurrent_Cron $cron */
        $cron = Mage::getModel('cc/recurrent_cron');

        $cron->chargeRecurringProfile();
    }
    ...
    public function redirectrecurrentAction()
    {
        $session = Mage::getSingleton('checkout/session');
        $session->setCcQuoteId($session->getQuoteId());
        $this->getResponse()->setBody($this->getLayout()->createBlock('cc/redirect_recurrent')->toHtml());
        $session->unsQuoteId();
    }

    public function redirectcryptoAction()
    {
        $session = Mage::getSingleton('checkout/session');
        $session->setCcQuoteId($session->getQuoteId());
        $this->getResponse()->setBody($this->getLayout()->createBlock('cc/redirect_crypto')->toHtml());
        $session->unsQuoteId();
    }
    ...
    function ipnrecurrentAction()
    {
        $objPmReq = $this->_processRequest('recurrent');
    ...
    function ipncryptoAction()
    {
        $objPmReq = $this->_processRequest('crypto');
    ...
    private function _processRequest($type = null)
    {
        ...
        if ($request->isPost()) {
            ...
            if ($envKey && $envData) {
                ...
                switch ($type) {
                    case 'recurrent':
                        $cc = Mage::getModel('cc/recurrent');
                        break;
                    case 'crypto':
                        $cc = Mage::getModel('cc/crypto');
                        break;
                    default:

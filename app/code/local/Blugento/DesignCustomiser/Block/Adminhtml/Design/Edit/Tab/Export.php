...
class Blugento_DesignCustomiser_Block_Adminhtml_Design_Edit_Tab_Export extends Mage_Adminhtml_Block_Widget_Form
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    protected function _prepareForm()
    {
        ...
        $expBlocksValue = 'blugento-header-block-1,blugento-header-block-2,blugento-footer-marketing';

        $expBlocksVariable = Mage::getModel('core/variable')->load('export-blocks', 'code');
        if ($expBlocksVariable->getId()) {
            $expBlocksValue = $expBlocksVariable->getPlainValue();
        }

        $fieldset->addField('export_blocks', 'text', array(
            'label'  => Mage::helper('blugento_designcustomiser')->__('Blocks to be exported/imported'),
            'value'  => $expBlocksValue,
            'name' => 'export_blocks',
            'after_element_html' => '<br/><small> ' . Mage::helper("blugento_designcustomiser")->__("Add multiple block identifier values separated by comma. No Save needed!"). '</small>',
            'tabindex' => 1
        ));

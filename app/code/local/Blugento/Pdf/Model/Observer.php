 ...
 */
/**
 * Blugento Pdf observer.
 *
 * @category  Blugento
 * @package   Blugento_Pdf
 * @author    Blugento Team <team@blugento.com>
 */
class Blugento_Pdf_Model_Observer
{
    /**
     ...
     *
     * @return Blugento_Pdf_Model_Observer
     */
    public function addInvoiceNotes(Varien_Event_Observer $observer)
    {
        ...
        $this->_addOrderComments($observer);
    ...
    private function _addShippingCountryNotes(Varien_Event_Observer $observer)
    {
        ...
        $shippingCountryNotes = Mage::helper('blugento_pdf/invoice')->getShippingCountryNotes($order);
    ...
    private function _addInvoiceDateNotice(Varien_Event_Observer $observer)
    {
        ...
        $result = $observer->getResult();
        $notes = $result->getNotes();
        $notes[] = Mage::helper('blugento_pdf')->__('Invoice date is equal to delivery date.');
        $result->setNotes($notes);
        return $this;
    }
    ...
    private function _addInvoiceMaturity(Varien_Event_Observer $observer)
    {
        ...
        if (!empty($maturity) || 0 < $maturity) {
            $maturity = Mage::helper('blugento_pdf')->__(
                'Invoice maturity: %s days', Mage::getStoreConfig('sales_pdf/invoice/maturity')
            );
        } elseif ('0' === $maturity) {
            $maturity = Mage::helper('blugento_pdf')->__('Invoice is payable immediately');
        }
    ...
    private function _addPaymentMethod(Varien_Event_Observer $observer)
    {
        if (Mage::getStoreConfig('sales_pdf/invoice/payment_method_position')
            != Blugento_Pdf_Model_System_Config_Source_Payment::POSITION_NOTE
        ) {
            return $this;
        }
        ...
        $result = $observer->getResult();
        $notes = $result->getNotes();
        $notes[] = Mage::helper('blugento_pdf')->__(
    ...
    private function _addShippingMethod(Varien_Event_Observer $observer)
    {
        ...
        if (empty($invoice) && empty($shipment)
            ...
            != Blugento_Pdf_Model_System_Config_Source_Shipping::POSITION_NOTE
    ...
    private function _addOrderComments(Varien_Event_Observer $observer)
    {
        if (!Mage::getStoreConfigFlag('sales_pdf/invoice/show_order_comments')) {
            return $this;
        }
        $invoice = $observer->getInvoice();
        ...
        foreach ($commentsObject as $commentObj) {
            if($commentObj->getComment()!='') {
                $notes[] = strip_tags($commentObj->getComment()) . " " . date('d/m/Y H:i', strtotime($commentObj->getCreatedAt()));
            }

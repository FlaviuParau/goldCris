 ...
 */
class Blugento_Pdf_Model_Engine_Invoice_Italia2 extends Blugento_Pdf_Model_Engine_Abstract
{
...
//        print_r($locale);exit;
        ...
        foreach ($invoices as $invoice) {
            ...
            if ($invoice->getStoreId()) {
                //Mage::app()->getLocale()->setDefaultLocale('it_IT');
    ...
    protected function insertTableHeader(&$page, $invoice, $order)
    {
        ...
        $columns['price_incl_tax'] = array(
            'label' => Mage::helper('blugento_pdf')->__('Price'),
            '_width' => 60,
            'currency' => true
        );
    ...
    protected function insertHeader(&$page, $order, $document)
    {
        ...
        $title = Mage::helper('blugento_pdf')->__('Data') . ' ' . Mage::helper('core')->formatDate($document->getCreatedAtDate(), 'medium', false);
        $page->drawText($title,
            ($page->getWidth() - $this->margin['left'] - $this->widthForStringUsingFontSize($title, $font, $this->fontSizes['big'])) / 2,
            $this->y, $this->encoding);
        $this->Ln();
    ...
    protected function _insertCustomerAddress(&$page, $order)
    {
        ...
        if (substr($billing[2],0,7)=='Cod TVA') {array_shift($billing);}
        ...
        foreach ($billing as $line) {
            ...
            $line=str_replace('Cod TVA','P.IVA',$line);

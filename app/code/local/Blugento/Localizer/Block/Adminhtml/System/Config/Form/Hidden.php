...
class Blugento_Localizer_Block_Adminhtml_System_Config_Form_Hidden extends Mage_Adminhtml_Block_System_Config_Form_Fieldset
{
    /**
    ...
    protected function _getFieldHtml($fieldset, $group)
    {
        ...
        if (strpos($id, 'store_id') !== false) {
            $data = Mage::helper('blugento_localizer')->getConfigScopeStoreId();
            $value = $data[0];
        } else
        if (strpos($id, 'website_id') !== false) {
            $data = Mage::helper('blugento_localizer')->getConfigScopeStoreId();
            $value = $data[1];
        }

        $field = $fieldset->addField($id . '_hidden', 'hidden', array(

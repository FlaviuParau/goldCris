...
class Blugento_Feeds_Model_Feed_Provider_Googleshopping extends Blugento_Feeds_Model_Feed_Abstract
{
    /**
     * @return string
     */
    ...
    {
        if (Mage::helper('blugento_feeds')->getGoogleShoppingDownloadOption() == 'xml') {
    ...
    public function setContentType()
    {
        ...
        } elseif (Mage::helper('blugento_feeds')->getGoogleShoppingDownloadOption() == 'csv') {
    ...
    {
        if (Mage::helper('blugento_feeds')->getGoogleShoppingDownloadOption() == 'xml') {
            $this->_feedSaveModel = 'blugento_feeds/feed_save_provider_googleshopping';
        } else {
            $this->_feedSaveModel = 'blugento_feeds/feed_save_provider_googleshoppingCsv';
        }
    ...
    public function getProducts($args)
    {
        ...
        $productAttributes = array('visibility', 'status', 'name', 'price', 'special_price', 'url_path', 'description',
            'short_description', 'image', 'special_from_date', 'special_to_date', 'manufacturer', 'meta_keyword');
        ...
        $sql = 'SELECT e.entity_id AS entity_id, e.sku AS sku, name.value AS name, de.value AS price, sp.value AS special_price, 
                    url.value AS url, image.value AS image, des.value AS description, shdes.value AS short_description, fro.value as special_from,
                    sto.value as special_to, sts.value as status, optv.value as manufacturer, keyword.value as keywords 
        ...
        foreach ($products as $key => $product) {
            ...
            $productsData[$key]['title'] = str_replace(',', ' ', $prodName);
            ...
            $productsData[$key]['subtitle'] = Mage::helper('cms')->getPageTemplateProcessor()->filter(str_replace(',', ' ', $product['short_description']));
            ...
            $productsData[$key]['g:price'] = number_format($product['price'], 2, '.', ',') . ' ' .  Mage::app()->getStore()->getCurrentCurrencyCode();

            $prodPrice = $this->_establishPrice($product['price'], $product['special_price'], $product['special_from'], $product['special_to']);
            $productsData[$key]['g:sale_price'] = number_format($prodPrice, 2, '.', ',') . ' ' .  Mage::app()->getStore()->getCurrentCurrencyCode();
            $productsData[$key]['g:availability'] = $product['status'] == 1 ? 'in stock' : 'out of stock';
//            $productsData[$key]['g:brand'] = $product['manufacturer'];
            $productsData[$key]['g:keywords'] = $product['keywords'];
            $productsData[$key]['g:item_address'] = Mage::getStoreConfig('general/store_information/address');
        }

        //if (Mage::helper('blugento_feeds')->getGoogleShoppingDownloadOption() == 'csv') {
        $tableHeaders = array(
            'g:id'            => 'ID',
            'g:mpn'           => 'ID2',
            'title'           => 'Item title',
            ...
            'g:product_type'  => 'Item category',
            'g:price'         => 'Price',
            'g:sale_price'    => 'Sale price',
            'g:availability'  => 'Availability',
            'g:keywords'      => 'Contextual keywords',
            'g:item_address'  => 'Item address',
        );

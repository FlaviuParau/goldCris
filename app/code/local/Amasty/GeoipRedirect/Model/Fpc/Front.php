...
//            $country = $location->getCountry();
            $country = $this->getLocationInfoByIp($ip);
...
/*            $pathInfo = explode('//', $path);
    ...
    protected function applyLogic($request)
    {
        ...
        switch ($applyLogic) {
            ...
            default:
                ...
                $redirectCookie = Mage::getSingleton('core/cookie')->get('redirect');
                $redirect = Mage::app()->getRequest()->getParam('redirect');
                if ($redirect && $redirect == 'no' && !$redirectCookie) {
                    $cookieDomain = Mage::app()->getRequest()->getHttpHost();
                    $cookieExpire = 3600;
                    $cookiePath = Mage::app()->getRequest()->getBasePath();
                    $cookieExpire = time() + $cookieExpire;
                    $cookieHttpOnly = $this->getDbConfig('web/cookie/cookie_httponly');
                    $cookieIsSecure = Mage::app()->getRequest()->isSecure();
                    setcookie('redirect', 'no', $cookieExpire, $cookiePath, $cookieDomain,
                        $cookieIsSecure, $cookieHttpOnly);
                }
                if (($redirect && $redirect == 'no') || $redirectCookie == 'no'){
                    $currentUrl = Mage::helper('core/url')->getCurrentUrl();
                    $url = basename($currentUrl);
                    $this->redirectAllowed = false;
                    $result = $url;
                }
        }
    ...
    private function getLocationInfoByIp($ip)
    {
        ...
        return !empty($result) ? $result['country'] : null;
    }
}

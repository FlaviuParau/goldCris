...
class Amasty_Label_Block_Adminhtml_Label_Edit_Tab_Condition extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        $byPrice = $fldPriceRange->addField('by_price', 'select', array(
            ...
            'options'   => array(           
                ...
                '4' => $hlp->__('Starting from Price (for Grouped Products)'),
                '5' => $hlp->__('Starting to Price (for Grouped Products)'),
            ),
        ));
		...
		if ($data) {
			...
			//set form values
            $form->setValues($data);
			$currentAttrValue = $attrValue->getValue();
			if ($attrValue->getType() == 'text' && isset($currentAttrValue[0])) {
			    $attrValue->setValue($currentAttrValue[0]);
			}
		}
    ...
    protected function getTree()
    {
        ...
        array_unshift($tree, array('value'=>'', 'label'=> $hlp->__('-- Do not apply --')));

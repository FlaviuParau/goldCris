...
class Amasty_Label_Block_Label extends Mage_Core_Block_Abstract
{
    ...
    protected function _toHtml()
    {
        ...
        $textStyle .= $this->_getTextRelatedStyles($customStyle);
        ...
        $html  = '<div class="' . 'amlabel-' . $label->getId()
            ...
            $html .= '  <div class="amlabel-txt2 ' . $positionClass . '" '
                . $textBlockStyle . ' ><div class="amlabel-txt" style="' . $textStyle . '">'
                . $label->getText() . '</div></div>';
        $html .= '</div>';

        return $html;
    }

    protected function _getTextRelatedStyles(&$customStyle)
    {
        $result = array();
        $massStyles = explode(';', $customStyle);
        $textTelatedStyles = array('font-', 'color', 'line-height');
        if ($massStyles) {
            foreach ($massStyles as $key => $style) {
                $style = trim($style);
                foreach ($textTelatedStyles as $textStyle) {
                    if (strpos($style, $textStyle) === 0) {
                        $result[] = $massStyles[$key];
                        unset($massStyles[$key]);
                         break;
                    }
                }
            }
            $customStyle = implode(';', $massStyles);
        }

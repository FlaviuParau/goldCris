...
class Amasty_Label_Adminhtml_AmlabellabelController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            if ($openTab) {
                Mage::getSingleton('core/cookie')->set('amasty_open_tab_input', $openTab, time() + self::   TIMER, '/');
            }
            ...
            try {
                ...
                $auto = false;
                if (!$model->getProdImageWidth() || !$model->getCatImageWidth()) {
    ...
    protected function prepareForSave($model)
    {
        ...
        if ($model->getProdImageWidth() > 100
            || $model->getCatImageWidth() > 100
        ) {
    ...
    protected function _checkAttribute($code)
    {
        $attribute = Mage::getModel('eav/entity_attribute')
            ->loadByCode(Mage::getResourceModel('catalog/product')->getTypeId(), $code);
        if (!$attribute->getUsedInProductListing()) {
            Mage::log('Check the `Used in Product Listing` property of the `'
                . $attribute->getFrontendLabel()
                . '` product attribute', null, 'Amasty_Label.log', true);
            $url = Mage::getModel('adminhtml/url')
                ->getUrl('adminhtml/catalog_product_attribute/edit', array('attribute_id' => $attribute->getId()));

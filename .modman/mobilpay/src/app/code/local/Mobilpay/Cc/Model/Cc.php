...
class Mobilpay_Cc_Model_Cc extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getOrderPlaceRedirectUrl()
    {
        ...
        if (!($e instanceof Exception)) {
            $this->getSession()->setMobilpayFormData($objPmReqCard->getEncData());
            $this->getSession()->setMobilpayFormKey($objPmReqCard->getEnvKey());

            $filename = $this->writeDataInFile($objPmReqCard->getEncData(), $objPmReqCard->getEnvKey());
        } else {
            $error = $e->getMessage();
            Mage::throwException($error);
        }
        ...
        if ($filename) {
            $url = Mage::getUrl('cc/cc/redirect', array(
                '_secure' => true, '_query'=>array('filename'=>$filename)));
        } else {
            $url = Mage::getUrl('cc/cc/redirect', array(
                '_secure' => true));
        }
    ...
    {
        Mage::getModel('cc/recurrent')->processRecurringProfile($objPmReq->objPmNotify->tokenId, $objPmReq->orderId);
    ...
    public function writeDataInFile($encData, $envKey)
    {
        $hash = Mage::helper('cc')->generateHash();
        $filename = $hash . '.txt';
        ...
        $file = fopen($directory . DS . $filename, 'w');
        fputs($file, $encData . PHP_EOL);
        fputs($file, $envKey);
        fclose($file);
    ...
    protected function _handleAuthorization($underVerification = true)
    {
        ...
        if (!$underVerification) {
            ...
            if (Mage::getStoreConfig('payment/mobilpay_cc/can_invoice')) {
...
*/
    ...
    protected function _handleCapture()
    {
        ...
        $this->_order->setStatus(Mage::getStoreConfig('payment/mobilpay_cc/shop_order_status'));
        $this->_order->save();
    }
    ...
    function getFormData()
    {
        ...
        return $this->getSession()->getMobilpayFormData();
    }

    function getFormKey()
    {

        return $this->getSession()->getMobilpayFormKey();
    }

...
class Blugento_Importer_Block_Adminhtml_System_Convert_Gui_Edit_Tab_Wizard extends Mage_Adminhtml_Block_Widget_Container
{
    ...
    {
        parent::__construct();
        $this->setTemplate('blugento/importer/profile/wizard.phtml');
    }
...
//    }
    ...
    public function getTaxClassesDefault()
    {
        $classSource = Mage::getModel('tax/class_source_product')->toOptionArray();

        $taxClasses = array();
        foreach ($classSource as $key=>$option) {
            $value = isset($option['value']) ? $option['value'] : '';
            $label = isset($option['label']) ? $option['label'] : '';
            $taxClasses[$value] = $label;
        }

        return $taxClasses;
    }

    public function getWebsitesDefault()
    {
        $websites = Mage::app()->getWebsites();

        $websitesResult = array();
        foreach ($websites as $website) {
            $websitesResult[$website->getCode()] = $website->getCode();
        }

        return $websitesResult;
    }

    public function getAttributesSetDefault()
    {
        ...
        foreach ($collection as $attributeSet) {
            $attributeSets[$attributeSet->getAttributeSetName()] = $attributeSet->getAttributeSetName();
        }

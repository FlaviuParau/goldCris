...
abstract class AW_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    public function getBlogUrl($route = null, $params = array())
    {
        ...
        // remove category from post url
        if (isset($params['cat']) && isset($params['post'])) {
            unset ($params['cat']);
            $blogRoute .= "/{$params['post']}";
        ...
        if (Mage::getStoreConfig('blog/blog/remove_last_slash')) {
            $url = substr($this->getUrl($blogRoute), 0, -1);
            //remove session param
            $sessionParamValue = Mage::app()->getRequest()->getParam('___SID');
            $url = str_replace('/?___SID=' . $sessionParamValue, '', $url);

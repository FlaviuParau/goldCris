...
class Blugento_Googleshopping_Helper_Data extends Blugento_Googleshopping_Helper_Write
{
    ...
    public function validateProduct($product, $config, $parent)
    {
        if (empty($config['skip_validation'])) {
            ...
            if ($config['exclude_configurable_parents'] == 1) {
                if ($product->getTypeId() == 'configurable') {
                    if (!$this->isSalable($product->getEntityId())) {
                        return false;
                    }
                }
            }
    ...
    {
        if ($dir) {
            $dir = Mage::getBaseDir('app') . DS . 'code' . DS . 'local' . DS . 'Blugento' . DS . $dir;

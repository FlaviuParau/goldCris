...
class Blugento_Googleshopping_Model_Googleshopping extends Blugento_Googleshopping_Model_Common
{

    /**
     * @var Blugento_Googleshopping_Helper_Data
     */
    public $helper;
    /**
    ...
    /**
     * Blugento_Googleshopping_Model_Googleshopping constructor.
     */
    ...
    public function generateFeed($storeId, $type = 'xml')
    {
        ...
        //create specific categories feeds
        $ios = [];
        if ($config['filter_enabled'] && $config['filter_specific_enabled']) {
            $specificCategories = explode(',', $config['filter_specific_cat']);
            foreach ($specificCategories as $specificCategory) {
                $config['file_name_temp'] = $this->getFileName('googleshopping', $storeId, $type, true);
                $config['file_name_temp'] = str_replace('.xml', '', $config['file_name_temp']) . '_' . $specificCategory . '.xml';
                ${'io' . $specificCategory} = $this->helper->createSpecificFeed($config);
                $ios[] = ${'io' . $specificCategory};
            }
        }
        ...
        //close specific categories feeds
        if ($config['filter_enabled'] && $config['filter_specific_enabled']) {
            $specificCategories = explode(',', $config['filter_specific_cat']);
            foreach ($specificCategories as $specificCategory) {
                $config['file_name_temp'] = $this->getFileName('googleshopping', $storeId, $type, true);
                ...
                $config['file_name_temp'] = str_replace('.xml', '', $config['file_name_temp']) . '_' . $specificCategory . '.xml';
                $config['file_name'] = str_replace('.xml', '', $config['file_name']) . '_' . $specificCategory . '.xml';

                $this->helper->closeSpecificFeed(${'io' . $specificCategory}, $config, $footer);
            }
        }
    ...
    public function getFeedConfig($storeId, $type = 'xml')
    {
        ...
        $config['media_url'] = $store->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB) . 'media/';
        ...
        $config['file_name_temp'] = $this->getFileName('googleshopping', $storeId, $type, true);
        $config['file_name'] = $this->getFileName('googleshopping', $storeId, $type);
        $config['file_path'] = Mage::getBaseDir() . DS . 'media' . DS . 'google-feed';
        $config['file_url'] = $config['media_url'] . 'google-feed' . DS . $config['file_name'];
        $config['filters'] = $this->helper->getSerializedConfigData('filter/advanced', $storeId);
        $config['version'] = (string)Mage::getConfig()->getNode()->modules->Blugento_Googleshopping->version;
        ...
        $config['filter_specific_enabled'] = $this->helper->getConfigData('filter/specific_category_enabled', $storeId);
        $config['filter_specific_cat'] = $this->helper->getConfigData('filter/specific_categories', $storeId);
        ...
        $config['only_one_price'] = $this->helper->getConfigData('advanced/only_one_price', $storeId);
        ...
        $config['exclude_configurable_parents'] = $this->helper->getConfigData('advanced/exclude_configurable_parents', $storeId);
     ...
     * @return int
     */
    public function getFeedData($products, $parents, $config, $parentAttributes, $prices, $io, $parentRelations, $ios = [])
    {
        $qty = 0;
        foreach ($products as $product) {
            ...
            if ($productData) {
                ...
                // add rows in specific categories feeds
                if ($config['filter_enabled'] && $config['filter_specific_enabled']) {
                    $specificCategories = explode(',', $config['filter_specific_cat']);
                    foreach ($specificCategories as $key => $specificCategory) {
                        if ($this->productInCategory($product->getEntityId(), $specificCategory)) {
                            $this->helper->writeRow($productRow->getData(), $ios[$key]);
                        }
                    }
                }
    ...
    protected function getCategoryData($productData, $config)
    {
        ...
        if (!empty($productData['g:google_product_category'])) {
            $category['g:product_type'] = $productData['g:google_product_category'];
        }
    ...
    public function getPrices($data, $confPrices, $product, $config, $parentId)
    {
        ...
        if ($config['only_one_price']) {
            if (isset($prices['g:price']) && isset($prices['g:sale_price'])) {
                $price = str_replace(' ' . $config['currency'], '', $prices['g:price']);
                $salePrice = str_replace(' ' . $config['currency'], '', $prices['g:sale_price']);
                if (floatval($price) >= floatval($salePrice)) {
                    $prices['g:price'] = $prices['g:sale_price'];
                    unset ($prices['g:sale_price']);
                }
            }
        }
    ...
    public function processUnset($productRow)
    {
        ...
        if ($productRow['g:product_type'] == $productRow['g:google_product_category']) {
            unset($productRow['g:google_product_category']);
        }
    ...
    protected function getFeedFooter($config, $timeStart, $processed = 0, $pages = 1)
    {
        ...
        $footer['extension'] = 'Blugento_Googleshopping';
    ...
    public function updateConfig($result, $type, $timeStart, $storeId)
    {
        ...
        if ($config['filter_enabled'] && $config['filter_specific_enabled']) {
            ...
            $specificCategories = explode(',', $config['filter_specific_cat']);
            foreach ($specificCategories as $specificCategory) {
                ${'url' . $specificCategory} = str_replace('.xml', '',  $result['url']) . '_' . $specificCategory . '.xml';
                $html .= '<br>' . sprintf(
                    '<a href="%s" target="_blank">%s</a><br/><small>On: %s (%s) - Products: %s/%s - Time: %s</small>',
     ...
     * @return bool
     */
    private function productInCategory($productId, $categoryId)
    {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_read');
        try {
            $result = $connection->query("SELECT *  FROM `catalog_category_product` WHERE `category_id` = $categoryId AND `product_id` = $productId;");
            $row = $result->fetch();
            $flag = !empty($row) ? true : false;
            return $flag;
        } catch (Exception $e) {
            Mage::logException($e);
        }

...
class AjaxLogin_IndexController extends Mage_Core_Controller_Front_Action
{
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                try {
                    ...
                    $result['redirect'] = !Mage::getStoreConfigFlag('customer/startup/redirect_dashboard') ? $this->_getRefererUrl() : Mage::getUrl('customer/account', array('_secure' => true));
                    $result['success'] = true;
                } catch (Mage_Core_Exception $e) {
                    switch ($e->getCode()) {

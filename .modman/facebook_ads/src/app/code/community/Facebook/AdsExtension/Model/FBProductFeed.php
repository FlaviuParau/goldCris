...
class FBProductFeed {
  ...
  const ATTR_ADDITIONAL_IMAGE_LINK = 'additional_image_link';
  ...
  protected function buildProductAttrText(
    ...
    switch ($attr_name) {
      ...
      case self::ATTR_SPECIAL_PRICE:
      case self::ATTR_GENDER:
      case self::ATTR_ADDITIONAL_IMAGE_LINK:
        if ((bool)$attr_value) {
  ...
  protected function buildProductEntry($product, $product_name) {
    ...
    if ($priceAttribute = Mage::getStoreConfig('Facebook_AdsExtension/product_feed/price_attribute')) {
        $price = Mage::getModel('directory/currency')->formatPrecision(
            $product->getData($priceAttribute),
    ...
    if ($this->conversion_needed) {
      ...
      if (Mage::getStoreConfig('Facebook_AdsExtension/product_feed/two_prices')) {
          if ($priceAttribute = Mage::getStoreConfig('Facebook_AdsExtension/product_feed/price_attribute')) {
              $price = Mage::getModel('directory/currency')->formatPrecision(
                  $product->getData($priceAttribute),
                  self::PRICE_PRECISION,
          ...
          if ($specialprice && ($product->getPrice() > $product->getFinalPrice())) {
              if ($today >= strtotime($specialPriceFromDate) && $today <= strtotime($specialPriceToDate . " +1 days") ||
                  $today >= strtotime($specialPriceFromDate) && is_null($specialPriceToDate) ||
                  is_null($specialPriceFromDate) && is_null($specialPriceToDate)) {

                  if ($specialPriceAttribute = Mage::getStoreConfig('Facebook_AdsExtension/product_feed/special_price_attribute')) {
                      $specialPrice = Mage::getModel('directory/currency')->formatPrecision(
                          $product->getData($specialPriceAttribute),
                          self::PRICE_PRECISION,
                          array('display' => Zend_Currency::NO_SYMBOL),
                          false);
                  } else {
                  ...
                  $items[self::ATTR_SPECIAL_PRICE] = $this->buildProductAttr('special_price',
                      sprintf('%s %s',
                          $this->stripCurrencySymbol($specialPrice),
                          Mage::app()->getStore($this->store_id)->getCurrentCurrencyCode()));
              } else {
                  $items[self::ATTR_SPECIAL_PRICE] = $this->buildProductAttr('special_price','');
              }
    ...
    $googleProductCategory = $product->getData('google_product_category');
    if ($this->getProductAttributeId('google_product_category')) {
        $googleProductCategory = $this->getGoogleProductCategory($product);
    }
...
//    $items[self::ATTR_GENDER] = $this->buildProductAttr(self::ATTR_GENDER, $gender);
      ...
      $items[self::ATTR_ADDITIONAL_IMAGE_LINK] = $this->buildProductAttr(
          self::ATTR_ADDITIONAL_IMAGE_LINK,
          $this->getMediaGallery($product));


      //extra attributes
      $extraAttributes = Mage::helper('Facebook_AdsExtension')->getAttributesMap();
      if (!empty($extraAttributes)) {
          foreach ($extraAttributes as $code => $extraAttribute) {
              $items[$extraAttribute] = $product->getAttributeText($code) ? : $product->getData($code);
          }
      }
    ...
    private function getProductAttributeId($attributeCode)
    {
        ...
        try {
            $result = $connection->query("SELECT attribute_id FROM eav_attribute
                WHERE attribute_code = '{$attributeCode}' AND entity_type_id = 4");
            $row = $result->fetch();
            return isset($row['attribute_id']) ? $row['attribute_id'] : null;
        } catch (Exception $e) {
            Mage::logException($e);
        }
    }
  ...
  private function writeProducts($io, $total_number_of_products, $should_log) {
    ...
    while ($count < $total_number_of_products) {
      ...
      $products = Mage::getModel('catalog/product')->getCollection()
        ...
        $restrictedProducts = [];
        if (Mage::getStoreConfig('Facebook_AdsExtension/product_feed/category_enabled')) {
            $restrictedCategory = Mage::getStoreConfig('Facebook_AdsExtension/product_feed/categories');
            $productsRestricted = $this->RestrictedProductsByCategory($restrictedCategory);
            foreach ($productsRestricted as $item) {
                $restrictedProducts[] = (int)$item['product_id'];
            }
        }
      ...
      foreach ($products as $product) {
        try {
          ...
          if (in_array($product_id, $restrictedProducts)) {
              continue;
          }
  ...
  private function getImageURL($product) {
    ...
    $image = $this->getBaseImage($product);
    ...
    if (!$image_url) {
      ...
      $image_url = str_replace('catalog/product//', 'catalog/product/', $image_url);
    }
    return $image_url;
  }
  ...
  private function getFinalPrice($product, $price = null) {
    ...
    if ($price === null) {
      ...
      $price = $this->_getFinalPrice($product);
    }
    if ($price === null) {
  ...
  private function getCategoryPath($product) {
    ...
    private function _getFinalPrice($product) {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_read');
        try {
            $result = $connection->query("SELECT final_price FROM catalog_product_index_price WHERE entity_id = {$product->getId()} AND customer_group_id = 0;");
            $row = $result->fetch();
    ...
    private function getBaseImage($product)
    {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_read');
        try {
            $result = $connection->query("SELECT value FROM catalog_product_entity_varchar WHERE entity_id = {$product->getId()} AND attribute_id = (SELECT attribute_id FROM eav_attribute WHERE entity_type_id = 4 AND attribute_code = 'image');");
            $row = $result->fetch();
            return $row['value'];
        } catch (Exception $e) {
            Mage::logException($e);
    ...
    private function getGoogleProductCategory($product)
    {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_read');
        try {
            $result = $connection->query("SELECT value FROM catalog_product_entity_varchar WHERE entity_id = {$product->getId()} AND attribute_id = (SELECT attribute_id FROM eav_attribute WHERE entity_type_id = 4 AND attribute_code = 'google_product_category');");
            $row = $result->fetch();
            return $row['value'];
        } catch (Exception $e) {
            Mage::logException($e);
    ...
    private function restrictedProductsByCategory($category)
    {
        $connection = Mage::getSingleton('core/resource')->getConnection('core_read');
        try {
            $result = $connection->query("SELECT product_id FROM catalog_category_product WHERE category_id = $category;");

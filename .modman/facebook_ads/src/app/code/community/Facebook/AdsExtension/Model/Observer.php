...
class Facebook_AdsExtension_Model_Observer {
  ...
  public function internalGenerateFBProductFeed(
    ...
    public function setLastUpdate()
    {
        ...
        $config->saveConfig('Facebook_AdsExtension/product_feed/run_time', date('Y-m-d H:i:s'), 'default', 0)->reinit();
        return $this;
    }
  ...
  {
      $feedLogException = Mage::getBaseDir('var') . '/log/' . FacebookAdsExtension::FEED_EXCEPTION;
      $logFile = Mage::getBaseDir('var') . '/log/' . FacebookAdsExtension::LOGFILE;
      $feedLogFile = Mage::getBaseDir('var') . '/log/' . FacebookAdsExtension::FEED_LOGFILE;
      $debugLogFile = Mage::getBaseDir('var') . '/log/' . FacebookAdsExtension::DEBUGMODE_LOGFILE;

      if (file_exists($feedLogException)) {
          unlink($feedLogException);
      }

...
class Facebook_AdsExtension_Block_Adminhtml_Diaindex
  ...
  public function fetchFeedSamples() {
    ...
    try {
      ...
      if ($productSamples && count($productSamples) > 0) {
      	foreach ($productSamples as $key => $productSample) {
	        $productSamples[$key]['description'] = htmlspecialchars(preg_replace('/[^A-Za-z0-9\-]/', ' ', $productSample['description']));
	        $productSamples[$key]['short_description'] = htmlspecialchars(preg_replace('/[^A-Za-z0-9\-]/', ' ', $productSample['short_description']));
        }
	    return json_encode($productSamples);
      } else {
	    return '[ /* no samples are generated */ ]';
      }
    } catch (Exception $e) {

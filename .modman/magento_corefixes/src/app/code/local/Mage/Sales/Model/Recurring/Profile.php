...
class Mage_Sales_Model_Recurring_Profile extends Mage_Payment_Model_Recurring_Profile
{
    ...
    public function createOrder()
    {
        ...
        $shippingTax = round(($shippingAmount / 119) * 19, 2);
        ...
        $billingAddress = Mage::getModel('sales/order_address')
            ->setData(is_string($this->getBillingAddressInfo()) ? unserialize($this->getBillingAddressInfo()) : $this->getBillingAddressInfo())
            ->setId(null);

        $shippingInfo = is_string($this->getShippingAddressInfo()) ? unserialize($this->getShippingAddressInfo()) : $this->getShippingAddressInfo();
        ...
        $orderInfo = is_string($this->getOrderInfo()) ? unserialize($this->getOrderInfo()) : $this->getOrderInfo();
        ...
        $qtyOrdered = $this->getInfoValue('order_info', 'items_qty') ? $this->getInfoValue('order_info', 'items_qty') : 1;

        $currencyCode = $this->getInfoValue('order_info', 'quote_currency_code') ?
    ...
    protected function _getRegularItem($itemInfo)
    {
        ...
        $qtyOrdered = $this->getInfoValue('order_item_info', 'qty') ? $this->getInfoValue('order_item_info', 'qty') : 1;

        $item = Mage::getModel('sales/order_item')
            ->setData(is_string($this->getOrderItemInfo()) ? unserialize($this->getOrderItemInfo()) : $this->getOrderItemInfo())

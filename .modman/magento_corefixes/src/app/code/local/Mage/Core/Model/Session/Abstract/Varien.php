...
class Mage_Core_Model_Session_Abstract_Varien extends Varien_Object
{
    ...
    public function start($sessionName = null)
    {
        ...
        if (Mage::app()->getFrontController()->getRequest()->isSecure() && empty($cookieParams['secure'])) {
            ...
            if (!isset($_SESSION[self::SECURE_COOKIE_CHECK_KEY])) {
                ...
                /*
                * NOTE:
                * This only renews the cookie for https requests
                */
                $tmp_secureCookieName = $sessionName . '_cid';
                $cookie->renew($tmp_secureCookieName, null, null, null, true);
            }
        }
    ...
    public function validate()
    {
        if (!isset($this->_data[self::VALIDATOR_KEY])) {
            ...
            if (!$this->_validate()) {
                Mage::log('SESSION - Destroyed current session. Validation Failed. With Data : ' . json_encode($this->_data, true), null, 'sessionDebug.log');
    ...
    protected function _validate()
    {
        ...
        if ($this->useValidateRemoteAddr()
            ...
            $this->recordFail($sessionData, $validatorData, 'ValidateRemoteAddr');
            return false;
        }
        if ($this->useValidateHttpVia()
            && $sessionData[self::VALIDATOR_HTTP_VIA_KEY] != $validatorData[self::VALIDATOR_HTTP_VIA_KEY]) {
            $this->recordFail($sessionData, $validatorData, 'ValidateHttpVia');
            return false;
        }
        ...
        if ($this->useValidateHttpXForwardedFor()
            ...
            $this->recordFail($sessionData, $validatorData, 'ValidateHttpXForwardedFor');
            return false;
        }
        if ($this->useValidateHttpUserAgent()
            ...
            $this->recordFail($sessionData, $validatorData, 'ValidateHttpUserAgent');
            return false;
        }

        if (get_class($this) == 'Mage_Core_Model_Session'
            && $this->useValidateSessionExpire()
            && isset($sessionData[self::VALIDATOR_SESSION_EXPIRE_TIMESTAMP])
            && $sessionData[self::VALIDATOR_SESSION_EXPIRE_TIMESTAMP] < time()) {
            $this->recordFail($sessionData, $validatorData, 'ValidateSessionExpire');
            return false;
        } else {
...
//            > $sessionData[self::VALIDATOR_SESSION_EXPIRE_TIMESTAMP] - $this->getCookie()->getLifetime()
            ...
            $this->recordFail($sessionData, $validatorData, 'ValidateSessionPasswordTimestamp');
            return false;
        }
     ...
     * @param $failFault
     */
    protected function recordFail($sessionData, $validatorData, $failFault)
    {
        Mage::log('SESSION - Failed : ' . $failFault, null, 'sessionDebug.log');
        Mage::log('SESSION - Session Data : ' . json_encode($sessionData, true), null, 'sessionDebug.log');
        Mage::log('SESSION - Validator Data : ' . json_encode($validatorData, true), null, 'sessionDebug.log');
    }
}
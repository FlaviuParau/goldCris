...
class Blugento_DesignCustomiser_Model_Scss_Save_Image_Scss extends Varien_File_Object
    implements Blugento_DesignCustomiser_Model_Scss_Save_Interface
{
     ...
     * @var string
     */
    protected $_filenameScss = '_variable-image.scss';
     ...
     * @return Blugento_DesignCustomiser_Model_Scss_Save_Scss
     */
    public function save(Varien_Data_Collection $collection) 
    {
        ...
        while (!$this->eof()) {
            ...
            foreach ($collection as $variable) {
                $scssDef = $variable->getScss();
                if (!$scssDef) {
                    $scssDef = '$' . $variable->getId() . ': [value]';
                }
                $scssDef = str_replace(' [value]', '', $scssDef);
    ...
    private function _setVariableValue(Blugento_DesignCustomiser_Model_Scss_Variable_Abstract $variable)
    {
        ...
        return str_replace('[value]', "'$value'", $scssDef) . ';' . PHP_EOL;
    }
}

...
class Blugento_DesignCustomiser_Model_Scss_Save_Image_Xml extends Varien_Simplexml_Config
    implements Blugento_DesignCustomiser_Model_Scss_Save_Interface
{
     ...
     * @var string
     */
    private $_xmlFileName = 'variable-image.xml';
    ...
    private function _setVariableValue(Blugento_DesignCustomiser_Model_Scss_Variable_Abstract $variable)
    {
        $scss =$this->_xml;
        $variableId = $variable->getId();
        $saveValue = $variable->getSaveValue();
        $value = isset($saveValue['name']) ? $saveValue['name'] : '';

        if ($scss->children() && $scss->$variableId) {

            if (!($scss->$variableId->value)) {
                $scss->$variableId->addChild('value', $value);
            } elseif ($value) {
                $scss->$variableId->value[0] = $value;
            } elseif (!$value) {
            ...
            $variableNode->addChild('value', $value);
        }

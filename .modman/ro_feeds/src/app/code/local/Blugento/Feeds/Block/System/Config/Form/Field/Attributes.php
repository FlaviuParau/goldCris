...
class Blugento_Feeds_Block_System_Config_Form_Field_Attributes
    extends Mage_Adminhtml_Block_System_Config_Form_Field_Array_Abstract
{
    ...
    protected function _prepareToRender()
    {
        $this->addColumn(
            'magento_code', array(
                'label'    => Mage::helper('blugento_feeds')->__('Magento Attribute'),
                'renderer' => $this->_getAttributeRenderer(),
            )
        );
        $this->addColumn(
            'feed_code', array(
                'label' => Mage::helper('blugento_feeds')->__('Feed Attribute Code'),
                'style' => 'width: 100px',
            )
        );

        $this->_addButtonLabel = Mage::helper('blugento_feeds')->__('Add Attribute');

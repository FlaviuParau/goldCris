    ...
    public function process($observer)
    {
        ...
        if ($isset) {
            ...
            if ($rule->getSimpleAction() == "by_percent") {
                $discount[$itemId] = [
                    'base_discount' => $result->getBaseDiscountAmount(),
                    'discount' => $result->getDiscountAmount(),
                    'percent' =>  $rule->getDiscountAmount()
                ];
                $ruleDiscount[$itemId] = $this->_limitMaxDiscount($discount, $rule, $itemId, $quote);
        ...
        $currentAmount = ($item->getRowTotalInclTax() ?: $item->getRowTotal()) - $item->getDiscountAmount() - $amountToDisplay;

...
Checkout.prototype = {
    ...
    reloadProgressBlock: function(toStep) {
        ...
        // TODO::refine this - Flaviu
        setTimeout(function(){
            jQuery('#co-payment-form fieldset + input').removeAttr("disabled");
        }, 1000);
    },
    ...
    setMethod: function(){
        ...
        else if($('login:login_in') && ($('login:login_in').checked || $('login:login_in').type == 'hidden')) {
            if (checkoutList.dataset.layout == 1) {
...
//            }
            ...
            if (checkoutList) {
                if (checkoutList.dataset.orientation == 2 && checkoutList.dataset.layout == 1) {
                    ...
                    var targetHeight   = currentSection.getElementsByClassName('step')[0].offsetHeight;
                    var titleHeight    = currentSection.getElementsByClassName('step-title')[0].offsetHeight;
                    var height         = titleHeight + targetHeight + 200;
                    checkoutList.setAttribute('style', 'min-height: ' + height + 'px');
                }
            }
        }
...
Shipping.prototype = {
    ...
    newAddress: function(isNew){
        if (isNew) {
            ...
            if (checkoutList) {
                if (checkoutList.dataset.orientation == 2 && checkoutList.dataset.layout == 1) {
                    var currentSection = document.getElementById('opc-shipping');
                    var targetHeight   = currentSection.getElementsByClassName('step')[0].offsetHeight;
                    var titleHeight    = currentSection.getElementsByClassName('step-title')[0].offsetHeight;
                    var height         = titleHeight + targetHeight;
                    checkoutList.setAttribute('style', 'min-height: ' + height + 'px');
...
//        $('billing:use_for_shipping_yes').checked = flag;
        ...
        if (checkoutList) {
            if (checkoutList.dataset.orientation == 2 && checkoutList.dataset.layout == 1) {
                var billingSection = document.getElementById('opc-billing');
                var shippingSection = document.getElementById('opc-shipping');
                var targetHeight   = (billingSection.getElementsByClassName('step')[0].offsetHeight != 0) ?
                    billingSection.getElementsByClassName('step')[0].offsetHeight : shippingSection.getElementsByClassName('step')[0].offsetHeight ;
                var titleHeight    = (billingSection.getElementsByClassName('step-title')[0].offsetHeight != 0) ?
                    billingSection.getElementsByClassName('step-title')[0].offsetHeight : shippingSection.getElementsByClassName('step-title')[0].offsetHeight;
                var height         = titleHeight + targetHeight;
                checkoutList.setAttribute('style', 'min-height: ' + height + 'px');
            }
        }
...
Payment.prototype = {
    ...
    changeVisible: function(method, mode) {
        ...
        if (checkoutList) {
            if (checkoutList.dataset.orientation == 2 && checkoutList.dataset.layout == 1) {
                var currentSection = document.getElementById('opc-payment');
                var targetHeight   = currentSection.getElementsByClassName('step')[0].offsetHeight;
                var titleHeight    = currentSection.getElementsByClassName('step-title')[0].offsetHeight;
                var height         = titleHeight + targetHeight + 30;
                checkoutList.setAttribute('style', 'min-height: ' + height + 'px');
            }
        }
    },
...
var DiscountCoupon = Class.create();

DiscountCoupon.prototype = {
    initialize: function(form, saveUrl, couponCode) {
        this.form = form;
    ...
    save: function() {
        ...
        if (this.validator.validate()) {
            var activeStep = checkout.accordion.currentSection.replace(/^opc-/, '');
            
            checkout.setLoadWaiting(activeStep);
            
            jQuery('#checkout-discount-coupon-box .button').hide();
            jQuery("#discount-coupon-please-wait").css({ opacity: 0.5 });
            jQuery('#discount-coupon-please-wait').show();
            
            var params = Form.serialize(this.form);
            
            if (activeStep) {
                params += '&active_step=' + activeStep;
            }
    ...
    cancelBtn: function () {
        jQuery('#discount-coupon-please-wait').hide();
        
        if (this.couponCode.length > 0) {
            jQuery('#btn-apply-coupon').show();
            jQuery('#btn-cancel-coupon').show();
        } else {
            jQuery('#btn-apply-coupon').show();
            jQuery('#btn-cancel-coupon').hide();
        }
    },
    ...
    nextStep: function(transport) {
        ...
        if (typeof response.active_coupon_code !== 'undefined') {
            this.couponCode = response.active_coupon_code;
            jQuery('#coupon-code').val(this.couponCode);
            
            this.cancelBtn();
        }
        
        checkout.setStepResponse(response);
        jQuery('#message-info').html(Translator.translate(message));
        
        setTimeout(function () {
            jQuery('#message-info').empty();
        }, 3000);
    }
}

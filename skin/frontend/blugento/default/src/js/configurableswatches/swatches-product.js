    ...
    this.settings.each(function(element){
        ...
        if(options) {
            ...
            for(var i=0;i<options.length;i++){
                ...
                element.options[index].setAttribute('data-attr-id', options[i].id);
...
Product.ConfigurableSwatches.prototype = {
    ...
    initialize: function(productConfig, config) {
        ...
        this._N.resetTimeout = setTimeout(function(){
            ...
            jQuery('.configurable-swatch-list').each(function() {
                if (jQuery('.last-selected')[0]) {
                    jQuery(this).find('li').last().find('a').attr('id', 'selected' + i++);
                    eventFire(document.getElementById('selected' + s++), 'click');
                } else if (jQuery('.first-selected')[0]) {
                    jQuery(this).find('li').first().find('a').attr('id', 'selected' + i++);
                    eventFire(document.getElementById('selected' + s++), 'click');
                }
            });
        }.bind(this), 300);
    ...
    attachOptEvents: function(opt) {
        ...
        if (opt._f.isSwatch) {
            opt._e.a.observe('click', function(event) {
                ...
                if (!jQuery('.disable-swatches-hover').length){
                    this.onOptionOver();
                }
    ...
    onOptionClick: function(attr) {
        ...
        if (opt) {
            if ((opt != attr._e._last.selectedOption) || (!opt._e.li.hasClassName('selected'))) {
                ...
                if (opt._f.isSwatch) {
                    ...
                    opt._e.ul.select('li').invoke('removeClassName','not-available');
        ...
        // Preselect single values
        if (jQuery('.preselect-swatch-1').length) {
            function preselectSwatches()
            {
                jQuery('.configurable-swatch-list').each(function () {
                    jQuery(opt._e.li).siblings().removeClass('single-value-available');
                    jQuery(opt._e.li).removeClass('not-available');
                    singleValues = jQuery(this).find('li:not(.not-available)').length;
    
                    if (singleValues < 2) {
                        jQuery(this).find('li:not(.not-available)').addClass('single-value-available');
                    }
                });
    
                jQuery('.single-value-available').each(function () {
                    var valueText = jQuery(this).find('.swatch-label span').text() || jQuery(this).find('.swatch-label img').attr('alt'),
                        valueId = jQuery(this).attr('data-id'),
                        selectAttr = jQuery(this).parents('.input-box').find('select').attr('id');
    
                    jQuery(this).parents('.swatch-attr').prev('.swatch-attr').find('.select-label').text(valueText);
                    jQuery(this).siblings().removeClass('selected');
                    jQuery(this).addClass('selected');
                    jQuery('#' + selectAttr).val(valueId);
    
                    eventFire(document.getElementById(selectAttr), 'change');
                });
            }

            preselectSwatches();

            jQuery('.configurable-swatch-list li.selected.not-available').each(function () {
                jQuery("#product-options-wrapper option:selected").removeAttr('selected');
                jQuery('.configurable-swatch-list li').removeClass('selected').removeClass('single-value-available');
                ...
                var valueId = jQuery(opt._e.li).attr('data-id'),
                    selectAttr = jQuery(opt._e.li).parents('.input-box').find('select').attr('id');

                jQuery(opt._e.li).addClass('selected');
                jQuery('#' + selectAttr).val(valueId);
    ...
    onOptionOver: function() {
        ...
        if (lastOpt && lastOpt._f.isSwatch) {
            ...
            if ((lastOpt._e.li).hasClassName('last-hover')) {
                lastOpt._e.li.removeClassName('last-hover');
                lastOpt._e.li.addClassName('not-available');
            }
        }
        // Set new hover class
        if (opt._f.isSwatch) {
            opt._e.li.addClassName('hover');
            if ((opt._e.li).hasClassName('not-available')) {
                opt._e.li.addClassName('last-hover');
                opt._e.li.removeClassName('not-available');
            }
        }
        ...
        if (!attr._f.isCustomOption) {
            ...
            //this.checkStockStatus( stockCheckOptions ); // commented this to fix bug for products with backorder
        }
    },
    /**
     *
    ...
    setStockStatus: function(inStock) {
        if (inStock) {
            this._E.availability.each(function(el) {
                var el = $(el),
                    inStockClass = jQuery('.availability .status');
                el.addClassName('in-stock').removeClassName('out-of-stock');
                if (jQuery(inStockClass).text() != Translator.translate('Backorder'))  {
                    el.select('span').invoke('update', Translator.translate('In Stock'));
                } else {
                    el.select('span').invoke('update', Translator.translate('Backorder'));
                }
            });
            ...
            this._E.cartBtn.btn.each(function(el, index) {
                ...
                // Check if fixed add to cart button it's visible on page
                if (jQuery('#product-addtocart-button-fixed').length) {
                    var fixBtn = document.getElementById('product-addtocart-button-fixed');
                    fixBtn.select('span span').invoke('update', Translator.translate(this._E.cartBtn.txt[index]));
                }
            }.bind(this));
        } else {
            this._E.availability.each(function(el) {
            ...
            this._E.cartBtn.btn.each(function(el) {
                ...
                if (jQuery('#product-addtocart-button-fixed').length) {
                    var fixBtn = document.getElementById('product-addtocart-button-fixed');
                    fixBtn.writeAttribute('title', Translator.translate('Out of Stock'));
                    fixBtn.select('span span').invoke('update', Translator.translate('Out of Stock'));
                }
            });
        }
    },
    ...
    updateSelect: function(attr) {
        ...
        eventFire(document.getElementById('attribute' + attr.id), 'change');
    },
    /**
